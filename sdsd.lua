local function main()
    ...
    local function setButtonTitleSafe(btn, text)
        pcall(function()
            if btn and type(btn.SetTitle) == "function" then
                btn:SetTitle(text)
            elseif btn and type(btn.SetDesc) == "function" then
                btn:SetDesc(text)
            end
        end)
    end
    ...
    local function saveGameSettings()
        ...
        savedSettings = {
            ...
            FogStart = Lighting2.FogStart,
            Brightness = Lighting2.Brightness,
            EnvironmentDiffuseScale = Lighting2.EnvironmentDiffuseScale,
            EnvironmentSpecularScale = Lighting2.EnvironmentSpecularScale,
            Ambient = Lighting2.Ambient,
            ExposureCompensation = Lighting2.ExposureCompensation,
            ClockTime = Lighting2.ClockTime,
            GeographicLatitude = Lighting2.GeographicLatitude,
            OutdoorAmbient = Lighting2.OutdoorAmbient,
            ShadowSoftness = Lighting2.ShadowSoftness,
            ColorShift_Top = Lighting2.ColorShift_Top,
            ColorShift_Bottom = Lighting2.ColorShift_Bottom,
        ...
        for _, v in ipairs(Lighting2:GetChildren()) do
            pcall(function()
    ...
    local function disablePotatoGraphics()
        ...
        pcall(function()
            ...
            Lighting2.GlobalShadows = savedSettings.GlobalShadows
            Lighting2.FogEnd = savedSettings.FogEnd
            Lighting2.FogStart = savedSettings.FogStart or 0
            Lighting2.Brightness = savedSettings.Brightness
            Lighting2.EnvironmentDiffuseScale = savedSettings.EnvironmentDiffuseScale
            Lighting2.EnvironmentSpecularScale = savedSettings.EnvironmentSpecularScale
            Lighting2.Ambient = savedSettings.Ambient
            Lighting2.ExposureCompensation = savedSettings.ExposureCompensation
            Lighting2.ClockTime = savedSettings.ClockTime
            Lighting2.GeographicLatitude = savedSettings.GeographicLatitude
            Lighting2.OutdoorAmbient = savedSettings.OutdoorAmbient
            Lighting2.ShadowSoftness = savedSettings.ShadowSoftness
            Lighting2.ColorShift_Top = savedSettings.ColorShift_Top
            Lighting2.ColorShift_Bottom = savedSettings.ColorShift_Bottom
    ...
    local function showAllTowers()
        ...
        for _, obj in ipairs(mainUI:GetChildren()) do
    ...
    local showAllTowersDescendantConnection = nil
    local showAllTowersPlayerGuiConnection = nil
    local lastShowAllCheck = 0

    local function startShowAllTowers()
        if showAllTowersConnection or showAllTowersDescendantConnection or showAllTowersPlayerGuiConnection then
            return
        end
        ...
        if playerGui then
            showAllTowersDescendantConnection = playerGui.DescendantAdded:Connect(function(obj)
                if not Settings.ShowAllTowers then return end
                if obj:IsA("GuiButton") or obj.Name == "Grid" or obj.Name == "Main" then
                    task.defer(showAllTowers)
                end
            end)
        else
            showAllTowersPlayerGuiConnection = player.ChildAdded:Connect(function(obj)
                if obj.Name == "PlayerGui" then
                    if showAllTowersPlayerGuiConnection then
                        showAllTowersPlayerGuiConnection:Disconnect()
                        showAllTowersPlayerGuiConnection = nil
                    end
                    task.defer(startShowAllTowers)
                end
            end)
        end
        ...
        showAllTowersConnection = RunService.Heartbeat:Connect(function()
            if not Settings.ShowAllTowers then return end
            local now = tick()
            if now - lastShowAllCheck < 0.15 then return end
    ...
    local function stopShowAllTowers()
        ...
        if showAllTowersDescendantConnection then
            showAllTowersDescendantConnection:Disconnect()
            showAllTowersDescendantConnection = nil
        end
        if showAllTowersPlayerGuiConnection then
            showAllTowersPlayerGuiConnection:Disconnect()
            showAllTowersPlayerGuiConnection = nil
    ...
    local function showBlackMarket()
        pcall(function()
            ...
            sg.Enabled = true
            local mainUI = sg:FindFirstChild("Main")
    ...
    local macroNameInputControl = nil
    local saveConfig
    ...
    do
        ...
        local MacroVirtualInputManager = game:GetService("VirtualInputManager")
        local MacroHttpService = game:GetService("HttpService")
        ...
        function Macro.StopRecording()
            ...
            if Settings.MacroAutoSaveEnabled and #Macro.Data > 0 then
                task.spawn(function()
                    pcall(function()
        ...
        local function HandleRecordedAction(Action)
            ...
            elseif key == Enum.KeyCode.Space and state == "Began" then
                ...
                pcall(function()
                    MacroVirtualInputManager:SendKeyEvent(state == "Began", key, false, game)
                end)
            end
        end
        ...
        local function SendRecordedMouse(Action)
            ...
            pcall(function() MacroVirtualInputManager:SendMouseMoveEvent(x, y, game) end)
            MacroRunService.Heartbeat:Wait()
            pcall(function() MacroVirtualInputManager:SendMouseButtonEvent(x, y, button, true, game, 0) end)
            MacroRunService.Heartbeat:Wait()
            pcall(function() MacroVirtualInputManager:SendMouseButtonEvent(x, y, button, false, game, 0) end)
        end
        ...
        function Macro.SaveNamed(name)
            ...
            local ok, err = pcall(function()
                if not isfolder(SaveFolder) then makefolder(SaveFolder) end
                local json = MacroHttpService:JSONEncode(SerializeMacro())
        ...
        function Macro.LoadNamed(name, silent)
            ...
            local okLoad, result = pcall(function()
                ...
                local data = MacroHttpService:JSONDecode(json)
        ...
        function Macro.SaveByBind()
            ...
            local name = Macro.ResolveName and Macro.ResolveName() or Macro.SelectedName
            if not name or name == "" then
            ...
            if ok then
                ...
                Settings.MacroSelectedName = name
        ...
        function Macro.SetupInput()
            ...
            inputBeganConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
                ...
                elseif input.UserInputType == Enum.UserInputType.MouseButton1
                    ...
                    local isOwnUI = false
                    if playerGui2 and Window and Window.Root then
                        local ok, guiObjects = pcall(function()
                            return playerGui2:GetGuiObjectsAtPosition(input.Position.X, input.Position.Y)
                        end)
                        if ok and guiObjects then
                            for _, obj in ipairs(guiObjects) do
                                local okCheck, result = pcall(function()
                                    return obj:IsDescendantOf(Window.Root)
                                end)
                                if okCheck and result then isOwnUI = true; break end
                            end
                        end
                    end
                    if not isOwnUI then
        ...
        function Macro.StartMovement()
            ...
            renderConn = MacroRunService.RenderStepped:Connect(function()
        ...
        function Macro.SaveSettings()
            pcall(function()
                if not isfolder(SaveFolder) then makefolder(SaveFolder) end
                writefile(SettingsFile, MacroHttpService:JSONEncode({
        ...
        function Macro.LoadSettings()
            ...
            local okLoad, data = pcall(function() return MacroHttpService:JSONDecode(readfile(SettingsFile)) end)
    ...
    local function SetupAutoCrates()
        ...
        local CrateRewards = {
            "Golden", "Diamond", "Mega Diamond", "Mega Golden",
            "Diamond Premium", "Golden Premium", "Cursed", "Drawn Credits",
            "Diamond Golden", "MultiVerse Diamond", "Multiverse Premium",
            "Golden Multiverse Premium", "Diamond Multiverse", "Mulitverse",
        }
        ...
        MainTab:AddSection("Auto Crates")
        ...
        TypeDD:OnChanged(function(v)
            for k, name in pairs(CrateOpenTypes) do
                if name == v then CrateState.OpenType = k; break end
            end
        end)
        ...
        AmountIn:OnChanged(function(v)
            CrateState.Amount = tonumber(v) or 1
        end)
        ...
        CrateToggle = MainTab:AddToggle("CrateAutoOpen", {
            ...
            Callback = function(v)
                if not v then
                    CrateState.Running = false
                    return
                end
                if CrateState.Running then return end

                CrateState.Running = true
                CrateState.Counter = 0

                task.spawn(function()
                    while CrateState.Running do
                        local ng, op = GetRemotes()
                        if ng and op then
                            pcall(function()
                                op:FireServer()
                                task.wait(0.05)
                                ng:FireServer(CrateState.OpenType, CrateState.Reward)
                            end)
                        end

                        CrateState.Counter = CrateState.Counter + 1

                        pcall(function()
                            ProgPara:SetDesc(
                                "Opened: " .. CrateState.Counter ..
                                (CrateState.Amount == -1 and " (∞)" or "/" .. CrateState.Amount)
                            )
                        end)

                        if CrateState.Amount ~= -1 and CrateState.Counter >= CrateState.Amount then
                            CrateState.Running = false
                            pcall(function()
                                CrateToggle:SetValue(false)
                                ProgPara:SetDesc("✅ Done: " .. CrateState.Counter .. " times")
                            end)
                            break
                        end
                    ...
                    if not CrateState.Running then
                        pcall(function()
                            ProgPara:SetDesc("🛑 Stopped at " .. CrateState.Counter .. " times")
                        end)
                    end
                end)
            end,
    ...
    local teleportButton = MainTab:AddButton({Title = "Teleport to Position (None)", Callback = function()
        ...
        if hrp and savedPosition then
            ...
            notifyUser("Teleported", "To " .. savedCoordsText, 2)
        else
            notifyUser("Error", "No saved position", 2)
        end
    end })
    ...
    MainTab:AddButton({Title = "Save Position", Callback = function()
        ...
        setButtonTitleSafe(teleportButton, "Teleport to Position " .. savedCoordsText)
    ...
    local camTeleportButton = MainTab:AddButton({Title = "Save Camera Position (None)", Callback = function()
        ...
        setButtonTitleSafe(camTeleportButton, "Save Camera Position (" .. savedCamText .. ")")
    ...
    MainTab:AddButton({Title = "Reset Camera Position", Callback = function()
        savedCamPos = nil
        savedCamText = "(None)"
        setButtonTitleSafe(camTeleportButton, "Save Camera Position (None)")
    ...
    OtherTab:AddToggle("ScriptAutoLoad", {
        Title = "Script Auto Load",
        ...
        Callback = function(v)
            ...
            if v then
                SetupAutoLoadTeleport()
                notifyUser("Script Auto Load", "Enabled", 2)
            else
                notifyUser("Script Auto Load", "Disabled", 2)
            end
        end
    })
    ...
    macroNameInputControl = MacroRecorderTab:AddInput("MacroNameInput", {
        Title = "Macro Name",
        Placeholder = "e.g. bee swarm 1",
        Default = Settings.MacroSelectedName or "",
        Callback = function(Text) macroNameInput = Text or "" end
    })

    if Settings.MacroSelectedName and Settings.MacroSelectedName ~= "" then
        macroNameInput = Settings.MacroSelectedName
    end

    pcall(function()
        if macroNameInputControl and type(macroNameInputControl.OnChanged) == "function" then
            macroNameInputControl:OnChanged(function(Text)
                macroNameInput = Text or ""
            end)
        end
    end)
    ...
    local macroSavedDropdown = MacroRecorderTab:AddDropdown("SavedMacros", {
        ...
        Multi = false,
        Default = Settings.MacroSelectedName ~= "" and Settings.MacroSelectedName or nil,
        Callback = function(opt) Macro.SelectedName = opt; Macro.RememberLast(opt); Settings.MacroSelectedName = opt or "" end
    })
    ...
    function Macro.ResolveName()
        local name = nil
        if macroNameInputControl then
            local ok, val = pcall(function() return macroNameInputControl:GetValue() end)
    ...
    function Macro.WriteToFile(name)
        local ok = pcall(function()
            local folder = "MacroRecorderData"
            if not isfolder(folder) then makefolder(folder) end
            writefile(folder .. "/" .. name .. ".json", HttpService:JSONEncode(Macro.SerializeData()))
        end)
        if ok then
            Macro.SelectedName = name
            Settings.MacroSelectedName = name
    ...
    function Macro.GetNextBackupName()
        local folder = "MacroRecorderData"
        if not isfolder(folder) then makefolder(folder) end
        local index = 1
        while isfile(folder .. "/autosavebackup" .. index .. ".json") do
            index = index + 1
            if index > 9999 then break end
        end
        return "autosavebackup" .. index
    end
    ...
    MacroRecorderTab:AddButton({ Title = "Save", Callback = function()
        ...
        if name then
            ...
            finalName = Macro.GetNextBackupName()
        end

        if Macro.WriteToFile(finalName) then
            Settings.MacroSelectedName = finalName
            pcall(function() Macro.SaveSettings() end)
    ...
    MacroRecorderTab:AddButton({ Title = "Remove", Callback = function()
        ...
        Macro.RemoveNamed(name)
        if Macro.SelectedName == name then
            Macro.SelectedName = nil
            Settings.MacroSelectedName = ""
    ...
    local function saveMacroSettingsToCurrentConfig()
        if ConfigApplying then return end
        if Settings.AutoSaveEnabled and currentConfig and currentConfig ~= "default" then
            pcall(function() saveConfig(currentConfig) end)
        end
    end

    macroAutoLoadToggle = MacroRecorderTab:AddToggle("MacroAutoLoadOnStart", {
        Title = "Auto-load macro on start",
        Description = "Loads the last saved macro automatically",
        Default = Settings.MacroAutoLoadOnStart,
        Callback = function(v) Settings.MacroAutoLoadOnStart = v; Macro.SaveSettings(); saveMacroSettingsToCurrentConfig() end
    })
    ...
    macroAutoStartToggle = MacroRecorderTab:AddToggle("MacroAutoStartPlayback", {
        Title = "Auto-start playback after load",
        Default = Settings.MacroAutoStartPlayback,
        Callback = function(v) Settings.MacroAutoStartPlayback = v; Macro.SaveSettings(); saveMacroSettingsToCurrentConfig() end
    })
    ...
    macroAutoSaveToggle = MacroRecorderTab:AddToggle("MacroAutoSaveEnabled", {
    ...
    macroAutoSaveIntervalSlider = MacroRecorderTab:AddSlider("MacroAutoSaveInterval", {
    ...
    macroDebugToggle = MacroRecorderTab:AddToggle("MacroDebugClicks", {
        Title = "Debug clicks in console",
        Default = Settings.MacroDebugClicks,
        Callback = function(v) Settings.MacroDebugClicks = v; Macro.SaveSettings(); saveMacroSettingsToCurrentConfig() end
    })

    macroCompensateToggle = MacroRecorderTab:AddToggle("MacroCompensateInset", {
        Title = "Compensate top bar (GuiInset)",
        Description = "Enable if clicks are offset on Y axis",
        Default = Settings.MacroCompensateInset,
        Callback = function(v) Settings.MacroCompensateInset = v; Macro.SaveSettings(); saveMacroSettingsToCurrentConfig() end
    })

    -- ===========================================================
    -- CONFIG SYSTEM
    -- ===========================================================
    CONFIG_FOLDER = "SkibidiConfigs"
    LAST_CONFIG_FILE = CONFIG_FOLDER.."/last.txt"
    AUTOLOAD_STATE_FILE = CONFIG_FOLDER.."/autoload.txt"
    ...
    local function loadDefault()
        ...
        pcall(function() configAutoLoadToggle:SetValue(Settings.AutoLoadEnabled) end)
        pcall(function() configAutoSaveToggle:SetValue(Settings.AutoSaveEnabled) end)
        ...
        setButtonTitleSafe(teleportButton, "Teleport to Position (None)")
        savedCamPos = nil
        savedCamText = "(None)"
        setButtonTitleSafe(camTeleportButton, "Save Camera Position (None)")
        updateMatchFieldsText()
    ...
    saveConfig = function(name)
        ...
        local data = {
            ...
            MacroBinds = {
                ...
                Play = MacroBinds.Play.Name,
                Save = MacroBinds.Save.Name,
            },
    ...
    function loadConfig(name)
        ...
        Settings.AutoSaveEnabled = data.AutoSaveEnabled or false
        Settings.AutoLoadEnabled = data.AutoLoadEnabled ~= false
        ...
        Settings.MacroAutoLoadOnStart = data.MacroAutoLoadOnStart ~= false
        Settings.MacroAutoLoadDelay = tonumber(data.MacroAutoLoadDelay) or 2
        Settings.MacroAutoStartPlayback = data.MacroAutoStartPlayback or false
        ...
        pcall(function()
            if macroAutoLoadToggle then macroAutoLoadToggle:SetValue(Settings.MacroAutoLoadOnStart) end
            if macroAutoLoadDelaySlider then macroAutoLoadDelaySlider:SetValue(Settings.MacroAutoLoadDelay) end
            if macroAutoStartToggle then macroAutoStartToggle:SetValue(Settings.MacroAutoStartPlayback) end
        ...
        if data.SavedPosition then
            ...
            setButtonTitleSafe(teleportButton, "Teleport to Position ("..x..","..y..","..z..")")
        else
            savedPosition = nil
            setButtonTitleSafe(teleportButton, "Teleport to Position (None)")
        end
        ...
        if data.SavedCamera then
            ...
            setButtonTitleSafe(camTeleportButton, "Save Camera Position ("..x..","..y..","..z..")")
        else
            savedCamPos = nil
            setButtonTitleSafe(camTeleportButton, "Save Camera Position (None)")
        end
    ...
    function rememberLastConfig(name)
    ...
    function AutoLoad()
        if isfile(AUTOLOAD_STATE_FILE) then
            local state = readfile(AUTOLOAD_STATE_FILE)
            if state == "0" then
                Settings.AutoLoadEnabled = false
                pcall(function() configAutoLoadToggle:SetValue(false) end)
    ...
    configAutoLoadToggle = ConfigTab:AddToggle("AutoLoad", {
        Title = "Auto Load",
        Description = "Automatically load the last config after rejoining",
        Default = Settings.AutoLoadEnabled,
        Callback = function(v)
            Settings.AutoLoadEnabled = v
            pcall(function() writefile(AUTOLOAD_STATE_FILE, v and "1" or "0") end)
            if not ConfigApplying and currentConfig ~= "default" then
                pcall(function() saveConfig(currentConfig) end)
            end
        end
    })
    configAutoSaveToggle = ConfigTab:AddToggle("AutoSave", {
        Title = "Auto Save",
    ...
    task.spawn(function()
        ...
        if not rememberedName or rememberedName == "" then
            pcall(function()
                local settingsFile = "MacroRecorderData/settings.json"
                if isfile(settingsFile) then
                    local data = HttpService:JSONDecode(readfile(settingsFile))
                    if data and data.SelectedName and data.SelectedName ~= "" then
                        rememberedName = data.SelectedName
                    end
                end
            end)
        end
        ...
        if not rememberedName or rememberedName == "" then
            rememberedName = Macro.GetLastSelected()
        end

        if rememberedName and rememberedName ~= "" then
